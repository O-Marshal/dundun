//
//  WalletCardCashListTableView.swift
//  dundun
//
...
class WalletCardCashListTableView: BaseViewController, CommonTableViewAnimationProtocol {
    ...
    override func netSuccess(result: String, identifier: String?) {
        if let jsonArray = MJson.json(result).array {
            datas += jsonArray.map({ MtableViewCommonModel(type: "m1", title: "卡卷返现", desc: $0["desc"].stringValue, money: $0["money"].floatValue.cfloatValue, time: $0["time"].stringValue) })
...
extension WalletCardCashListTableView: UITableViewDataSource, UITableViewDelegate {
