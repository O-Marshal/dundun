//
//  PersonalAddressNewViewController.swift
//  dundun
//
...
class PersonalAddressNewViewController: BaseViewController, CommonTableViewAnimationProtocol {
    ...
    override func netSuccess(result: String, identifier: String?) {
        Notify.show(Murmur: "收货地址新增成功", theme: NotiTheme.Success)
...
extension PersonalAddressNewViewController {
    ...
    func upload() {
        print("name:\(name);pho:\(pho);adr:\(adr)")
        if name == nil {
        ...
        postWithLogin("http://dundun.mog.name/adr/create", params: ["name": name!, "pho": pho!, "adr": adr!])
...
// MARK: - TbaleView -
extension PersonalAddressNewViewController: UITableViewDataSource, UITableViewDelegate {
...
extension PersonalAddressNewViewController: PersonalAdrCellNewProtocol {
