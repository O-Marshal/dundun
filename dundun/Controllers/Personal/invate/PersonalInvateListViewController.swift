//
//  PersonalInvateListViewController.swift
//  dundun
//
...
class PersonalInvateListViewController: BaseViewController, CommonTableViewAnimationProtocol {
    ...
    override func initEvent() {
        ...
        postWithLogin("http://dundun.mog.name/active/invitatelist", params: ["page": 1])
    }
    
    override func netSuccess(result: String, identifier: String?) {
        if let jsonArray = MJson.json(result).array {
            datas += jsonArray.map({ MtableViewCommonModel(type: "invitate", title: "签到记录", desc: $0["desc"].stringValue, money: 0, time: $0["time"].stringValue) })
...
extension PersonalInvateListViewController: UITableViewDataSource, UITableViewDelegate {
